...
class TestSales(BaseTest):    
    ...
    def test_post_sale_with_admin_token(self):
        resp = self.test_client.post(
            "/api/v2/sales",
            data=json.dumps([{
                "name": "milk 500ml",
        ...
        self.assertEqual(resp.status_code, 401)
        response = json.loads(resp.data)
        self.assertEqual(response["message"], "only a store attendant can post a sale")
    ...
    def test_get_one_sale_user(self):
        resp = self.test_client.get("/api/v2/sales/1",
                                    headers={
                                        "access_token": self.user_token
                                    })
    ...
    def test_get_one_sale_user2(self):
        resp = self.test_client.get("/api/v2/sales/1",
                                    headers={
